...
module.exports.execute = async (user) => {
    ...
    if (user !== "prodzpod") {
        let announcement_mini = `[🌙] GreenFeed 🟢 ${stream.user_name} has started a ${stream.game_name} stream, check it out at https://twitch.tv/${user} !`;
        let announcement_full = `<@&1270494916325277727>\n[🟢] **${stream.user_name}** has started a **${stream.game_name}** stream, come hang out!\nhttps://twitch.tv/${user}`;
    ...
    send("web", "offline", WASD.toString(user), stream);
    return [0, ""];
}