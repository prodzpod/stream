...
const { numberish, realtype, unentry } = require("../../../../common");
...
module.exports.result = async (currentTokens, index, tokens, offset, amount, stack) => {
    ...
    let n = await transformIfType(currentTokens, 
        ...
        [[TYPE.list, null, TYPE.list], (a, _, b) => [TYPE.list, a.filter(x => b.some(y => x.is(y)))]],
        [[TYPE.dict, null, TYPE.list], (a, _, b) => [TYPE.dict, unentry(Object.keys(a).filter(k => b.map(x => toString(x)).includes(k)).map(x => [x, a[x]]))]],
        [[TYPE.dict, null, TYPE.dict], (a, _, b) => [TYPE.dict, unentry(Object.keys(a).filter(k => b[k] && a[k].is(b[k])).map(x => [x, a[x]]))]],
