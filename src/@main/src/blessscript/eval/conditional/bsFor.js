...
module.exports.condition = (currentToken, index, tokens, stack) => currentToken.is(TYPE.operator, "for");
...
module.exports.result = async (currentTokens, index, tokens, offset, amount, stack) => {
    ...
    let lvalue = splitTokens(currentTokens.slice(2, idx - 1), new Token(TYPE.operator, ";"));
    lvalue = lvalue.slice(0, 3);
    lvalue[0] ??= []; lvalue[1] ??= [new Token(TYPE.bool, true)]; lvalue[2] ??= [];
    let rvalue = currentTokens.slice(idx);
    return [[new Token(TYPE.operator, "forLATE"), new Token(TYPE.function, {args: {}, fn: lvalue[0]}), new Token(TYPE.function, {args: {}, fn: lvalue[1]}), new Token(TYPE.function, {args: {}, fn: lvalue[2]}), new Token(TYPE.function, {args: {}, fn: rvalue})], stack];
}
