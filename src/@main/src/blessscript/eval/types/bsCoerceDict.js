...
const { transformIfType, toBool, toNumber, toString, toList, toDict } = require("../bsEvalUtil");

// int
module.exports.priority = 20;
// (Token, int, [Token], StackData) => bool
module.exports.condition = (currentToken, index, tokens, stack) => currentToken.is(TYPE.operator, "dict");
...
module.exports.result = (currentTokens, index, tokens, offset, amount, stack) => {
    let n = toDict(currentTokens[1]); if (n?.type === TYPE.error) return [[n], stack];
    return [[new Token(TYPE.dict, n)], stack];
}