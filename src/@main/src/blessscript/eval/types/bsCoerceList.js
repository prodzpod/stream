...
const { transformIfType, toBool, toNumber, toString, toList } = require("../bsEvalUtil");

// int
module.exports.priority = 20;
// (Token, int, [Token], StackData) => bool
module.exports.condition = (currentToken, index, tokens, stack) => currentToken.is(TYPE.operator, "list");
...
module.exports.result = (currentTokens, index, tokens, offset, amount, stack) => {
    let n = toList(currentTokens[1]); if (n?.type === TYPE.error) return [[n], stack];
    return [[new Token(TYPE.list, n)], stack];
}