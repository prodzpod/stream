...
const { random, Math, unentry, nullish, WASD, realtype } = require("../../common");
const { log } = require("../../commonServer");
const { args } = require("../chat/chat");

module.exports.predicate = "!clone";
module.exports.permission = false;
module.exports.execute = async (_reply, from, chatter, message, text) => {
    let _args = args(text);
    let forceCategory = null;
    let forceHexes = null;
    if (prompts.some(x => x._name === _args[0])) {
        forceCategory = _args[0];
        _args = _args.slice(1);
    }
    if (realtype(_args[0]) === "array") {
        forceHexes = _args[0];
        _args = _args.slice(1);
    }
...
module.exports.prompt = async (prompt, forceCategory = null, forceHexes = null) => {
    let category = prompts.findIndex(x => x._name === forceCategory);
    if (category === -1) category = random(unentry(Object.entries(prompts).map(x => [x[0], x[1].weight])));
    let promptSelected = prompts[category];
    let ret = { ...promptSelected.chatter(data().user["140410053"]) };
    ret.hexes = forceHexes ?? [];
    if (OVERRIDE.length) { ret.sigil = SIGIL_NORMAL + SIGIL_CLONE; ret.res = OVERRIDE; }
        ...
        for (let name of ret.hexes) {
            let hex = hexes.find(x => x.name === name);
            if (!hex) continue;
...
const prompts = [
    {
        ...
        chatter: prod => ({
            ...
            sigil: SIGIL_NORMAL + "🔏" + SIGIL_CLONE,
            system: _ => "",
            postprocess: _ => " ".repeat(Math.floor(random(1, 10))),
        }),
    },{
