...
module.exports.execute = (_reply, from, chatter, message, text, reply) => {
    ...
    log("Redeeming:", cmd, chatter.twitch.name, chatter.twitch.color, x1, y1, x2, y2, ..._args.slice(4));
    _reply("it worked");
    send("gizmo", cmd, chatter.twitch.name, chatter.twitch.color, x1, y1, x2, y2, ..._args.slice(4)); return [0, ""];
}
