...
module.exports.execute = (_reply, from, chatter, message, text, reply) => {
    ...
    return src().genericXY.execute(_reply, from, chatter, message, WASD.pack("!idoldream", x, y, _args[2] ?? `${chatter.twitch.name} is following their Idol Dream`, chatter.twitch.profile), reply);
}