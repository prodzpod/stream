...
let RULE = {
    ...
    "2kki": {
        rule: name => name.endsWith(" - EasyRPG Player"),
        name: "%NAME%:SDL_app:RPG_RT.exe",
        window: null,
    },
    nikki: {
...
module.exports.execute = async (...args) => {
    ...
    for (let window of update) {
        ...
        if (window.x !== undefined || window.y !== undefined || window.w !== undefined || window.h !== undefined) {
            let o = {};
            if (window.x) {
                o.positionX = window.x;
                RULE[k].window.x = window.x;
            }
            if (window.y) {
                o.positionY = window.y;
                RULE[k].window.y = window.y;
            }
            if (window.w) {
                // o.scaleX = window.w;
                RULE[k].window.w = window.w;
            }
            if (window.h) {
                // o.scaleY = window.h;
                RULE[k].window.h = window.h;
            }
