...
module.exports.idn = o => {
    ...
    switch (typeof o) {
        ...
        case 'string':
            let ret = "";
            for (let i = 0; i < o.length; i += 2) { ret += this.idnChar((Number.isNaN(o.charCodeAt(i)) ? 0 : o.charCodeAt(i) << 8) + (Number.isNaN(o.charCodeAt(i + 1)) ? 0 : o.charCodeAt(i + 1))); }
            return ret;
    }
...
module.exports.deidn = str => {
    let ret = "";
    for (let i = 0; i < str.length; i++) {
        let temp = this.deidnChar(str[i], str[i + 1]);
        if (temp[1]) i++;
...
module.exports.idnChar = x => {
    for (let region of this.IDN_REGIONS) {
