...
let main = undefined, commands = {}, waitList = {};

const MODULE_NAME = "discord"; //! change this every folder
...
module.exports.init = async () => {
    ...
    await new Promise(resolve => {
        main = new WebSocket(`ws://localhost:339/${MODULE_NAME}`);
        main.on("open", () => {
        ...
        main.on("message", async msg => {
            ...
            main.send(WASD.pack(id, "respond", status, res));        
        });
        main.on("close", e => console.log(e));
        main.on("error", e => console.log(e));
    });
    return 0;
}
...
module.exports.send = (...args) => {
    if (main?.readyState !== 1) { console.error("MAIN DOES NOT EXIST: ", ...args); return; } //! no main to send logs
    ID++; main.send(WASD.pack(ID, ...args));
