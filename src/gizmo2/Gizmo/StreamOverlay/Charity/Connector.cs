...
namespace CheatEngine
{
    ...
    public class CheatEngineLibrary
    {

        // Static invocations.
        public static IntPtr LoadLibraryW(string strLib) => LoadLibrary(strLib);
        public static IntPtr LoadLibrary(string strLib)
        {
            var a = PInvoke.Kernel32.LoadLibrary(strLib);
            if (a.IsInvalid)
            {
        ...
        public static IntPtr GetProcAddress(IntPtr iModule, string strProcName) => PInvoke.Kernel32.GetProcAddress(new(iModule), strProcName);
 ...
 bool unicode, bool casesensitive, bool percentage, bool compareToSavedScan, [MarshalAs(UnmanagedType.BStr)] string savedscanname);
        ...
        {
            libInst = LoadLibraryW(FileP.Path("ce-lib64.dll"));
            if (libInst != IntPtr.Zero)
            {
                Logger.Info("loaded");
                loadFunctions();
            }
            else Logger.Error("error, can't load the library");
        }
    }
