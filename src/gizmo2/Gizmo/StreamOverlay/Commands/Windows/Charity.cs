    ...
    public class Charity : Command
    {
        public override async Task<object?[]?> Execute(params object?[] args)
        {
            ...
            StreamOverlay.DrawResolve.Add(new(Image.Load(profile), texture =>
            {
                ...
                var i = RaidWindow.New(new(960, 540), $"{name} donated {value.Value} {currency} to VTubers Against ICE!", sprite, MathP.Max(1, (int)value.Value), "<wave>" + comment);
