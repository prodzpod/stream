...
const { getSocketsServer, incrementData, data } = require('../../@main/include');
const { String, WASD } = require('../../@main/util_client');
...
module.exports.execute = async (args, __, ___) => {
    ...
    incrementData(`user.${args[1]}.point`, args[2].length / 10);
    getSocketsServer('model')?.send(WASD.pack('discord', 0, 'chat', 
        `lib/icons_win2k_sp4_en/${TODO_TESTSTREAM_ICONS[String.hashCode(args[1]) % TODO_TESTSTREAM_ICONS.length]}`,
        (tags.color ?? '#000000').slice(1), args[1], args[2]));
    return 0;
}