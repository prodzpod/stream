const { STREAMER_ID, LALA_ID, FORREST_ID, ELLG_ID, BUG_ID } = require("../common");
...
module.exports.execute = req => {
    if (req.chatter_user_id == STREAMER_ID && req.message.text.startsWith("!") && clonkCommands.every(x => !x.includes(req.message.text))) send("chat", {
        ...
        message: { id: req.message_id, text: req.message.text, channel: BUG_ID, emotes: [] },
    });
    else send("clonkchat", req.message.text, req.chatter_user_name, BUG_ID, req.chatter_user_id, req.message_id);
}